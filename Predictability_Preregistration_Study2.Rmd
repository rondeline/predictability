---
title: "Predictability Study 2 Preregistration"
...
Participants will be recruited through an IRB-approved online research platform. Participants will be between 3;0 years and 3;11 years at time of test, must have normal or corrected-to-normal vision, typical hearing, no reported cognitive or neurodevelopmental delays/disorders, and must be exposed to English at least 75% of the time at home.

### Sample Size

We will recruit will a total of ## participants for study 2.45 participants to the predictable condition and 45 participants to the continuous condition. 

### Sample Size Rationale

We ran a Bayesian power analysis 
...
#read in the data
data <- read.csv("pred_data_log.csv")

#tidy data/include only data from 3 y/os
tidy_data <- data %>% 
  clean_names() %>% 
  filter(included == "Yes",
         age == 3) %>% 
...
#determine ES for IP-CO
  #Cohen's d = (mu_1 - mu2 / pooled sd)
mu_IP1 <- mean(tidy_data$mean_accuracy[tidy_data$condition == "IP"])
mu_CO1 <- mean(tidy_data$mean_accuracy[tidy_data$condition == "CO"])
pooled_sd1 <- sd(tidy_data$mean_accuracy)

d1 <- (mu_IP1 - mu_CO1) / pooled_sd1

#determine ES for IP-IUP
df2 <- expand_grid(subject_id = 1:20,
                         condition = c("IP", "IUP"),
                         trial_number = 1:12) %>% 
  group_by(condition) %>% 
  mutate(accuracy = ifelse(condition == "IP",
                          rbinom(n = n(), size = 1, prob = 0.805),
                          rbinom(n = n(), size = 1, prob = 0.748)))
tidy_df2 <- df2 %>% 
  group_by(subject_id, condition) %>% 
  summarise(mean_accuracy2 = mean(accuracy))

mu_IP2 <- mean(tidy_df2$mean_accuracy2[tidy_df2$condition == "IP"])
mu_IUP2 <- mean(tidy_df2$mean_accuracy2[tidy_df2$condition == "IUP"])
pooled_sd2 <- sd(tidy_df2$mean_accuracy2)

d2 <- (mu_IP2 - mu_IUP2) / pooled_sd2
